#!/usr/bin/env python3
"""
C++ accelerated DR-EKF TAC with MOSEK, FW, or FW-Exact solver.
...
class DR_EKF_TAC_CPP(BaseFilter):
    def __init__(self, T, dist, noise_dist, system_data, B,
                 ...
                 shared_noise_sequences=None,
                 input_lower_bound=None, input_upper_bound=None,
                 solver="mosek",
                 dr_update_period=1,
                 fw_beta_minus1=1.0, fw_tau=2.0, fw_zeta=2.0, fw_delta=0.05,
                 fw_max_iters=20, fw_gap_tol=1e-4,
                 fw_bisect_max_iters=30, fw_bisect_tol=1e-6,
                 fw_exact_iters=8, fw_exact_gap_tol=1e-6,
                 oracle_exact_tol=1e-8, oracle_exact_max_iters=60,
                 dynamics_type="ct3d", dt=0.2, sensor_pos=None):
        ...
        if solver not in ("fw", "fw_exact", "mosek"):
            raise ValueError(f"solver must be 'fw', 'fw_exact', or 'mosek', got '{solver}'")
        ...
        self._cpp = dr_ekf_cpp.DR_EKF_TAC(
            self._cpp_dynamics,
