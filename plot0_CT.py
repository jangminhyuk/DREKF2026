...
RESULTS_DIR = os.path.join("results", "EKF_comparison_CT_cpp")
...
# Plotting helpers
# ---------------------------------------------------------------------------
def _uncertainty_tube(ax, x_mean, y_mean, x_std, y_std, color, alpha=0.3, scale=0.1):
    """Draw an uncertainty tube around a 2-D trajectory."""
    ...
    dx /= norms;  dy /= norms
    perp_x, perp_y = -dy, dx
    ux = x_mean + perp_x * std_mag
    uy = y_mean + perp_y * std_mag
    lx = x_mean - perp_x * std_mag
    ly = y_mean - perp_y * std_mag
    tube_x = np.concatenate([ux, lx[::-1]])
    tube_y = np.concatenate([uy, ly[::-1]])
    ax.fill(tube_x, tube_y, color=color, alpha=alpha)


def _plot_single_trajectory(ax, tdata, color, label, linestyle='-', lw=2.5,
                            show_true=True, show_tube=True):
    """Plot estimated trajectory (with optional tube & true reference) on *ax*."""
    mean = tdata['mean']
    std = tdata['std']
    x_m, y_m = mean[:, 0], mean[:, 1]
    if show_tube:
        _uncertainty_tube(ax, x_m, y_m, std[:, 0], std[:, 1], color)
...
def plot_individual_trajectories(trajectory_data, dist):
    ...
    for filt, tdata in trajectory_data.items():
        ...
        _plot_single_trajectory(ax, tdata, color, display_name(filt))
        ...
        fname = f"traj_2d_{filt}_{dist}.pdf"
        plt.savefig(os.path.join(RESULTS_DIR, fname), dpi=300, bbox_inches='tight')
        saved.append(fname)
        plt.close(fig)

    print(f"Saved {len(saved)} individual trajectory plots to {RESULTS_DIR}/")


# ---------------------------------------------------------------------------
# 2) Combined trajectory subplot (all filters in one figure)
# ---------------------------------------------------------------------------
def plot_combined_trajectories(trajectory_data, dist):
    """All filters in one figure with subplots."""
    _ensure_dir(RESULTS_DIR)
    ...
    for idx, filt in enumerate(available_filters):
        ...
        _plot_single_trajectory(ax, tdata, color, display_name(filt), show_true=True, show_tube=True)
    ...
    legend_els = [
        ...
        plt.Rectangle((0, 0), 1, 1, fc='gray', alpha=0.3, label='1-std tube'),
    ...
    plt.savefig(os.path.join(RESULTS_DIR, fname), dpi=300, bbox_inches='tight')
    plt.close(fig)
    print(f"Saved combined trajectory figure: {RESULTS_DIR}/{fname}")
...
def plot_mse_heatmaps(all_results, dist):
    ...
    # CDC-based filters (theta_x × theta_v)
    ...
    for filt in cdc_filters:
        ...
        # Extract theta combinations and MSE values
        data = {}
...
def create_violin_plots(optimal_results, results_path, dist):
    ...
    ax.set_title(f'MSE Distribution at Optimal Parameters ({dist.title()})', fontsize=16)
    ax.set_yscale('log')
...
def plot_mse_bar_chart(optimal_results, dist):
    ...
    ax.set_title(f'MSE Comparison at Optimal Parameters ({dist.title()})', fontsize=16)
...
def main():
    parser = argparse.ArgumentParser(
        description='Visualise EKF vs DR-EKF comparison results (C++ accelerated)')
    ...
    print("=" * 80)
    print("VISUALIZATION (2D Coordinated Turn) - C++ Accelerated")
    print("=" * 80)
    ...
    try:
        ...
        if trajectory_data:
            ...
            print("\nCreating combined trajectory figure...")
